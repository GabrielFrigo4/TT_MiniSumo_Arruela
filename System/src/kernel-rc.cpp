...
void behavior_forward(
    const uint8_t sense, const uint8_t speed_modifier, const uint8_t sense_modifier
);
...
void update()
{
	...
	if (controller.l2 && controller.l2_value >= BUTTON_TRIGGER)
	{
		...
		behavior_forward(
		    ...
		    tt::utilities::abs(controller.l_stick_x * 0.9)
		);
		update_engine();
		return;
	}
...
void special_diagonal()
{
	...
	{
		direction_t
		    direction = (controller.r_stick_x < 0 ? direction_t::left : direction_t::right);
		switch (direction)
		{
		case direction_t::left:
			tt::engine::move(TT_ENGINE_BACK_FULL, TT_ENGINE_FRONT_FULL);

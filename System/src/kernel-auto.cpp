...
void setup_task()
{
	...
	xTaskCreatePinnedToCore(
	    sensor_task, "SensorTask", STACK_SIZE, nullptr, 16, &SensorTask, PRO_CPU_NUM
	);
	vTaskDelay(512);
}
...
void setup_connect()
{
	...
	tt::serial::printf(
	    STRLN("Digite '%c' para Mostrar os Comandos Disponíveis!"), COMMAND_HELP
	);
}
...
void setup_estrategia()
{
	...
	while (bluetooth_input_char != COMMAND_SETUP)
	{
		...
		case COMMAND_CHECK:
			...
			tt::serial::printf(
			    STRLN("| Direção    | %s |"),
			    (direction_init == direction_t::right ? "direita " : "esquerda")
			);
			tt::serial::printf(STRLN("+------------+----------+"));
			break;
...
void procura_padrao(const uint8_t velocidade_giro)
{
	...
	{
		tt::engine::brake();
		for (uint8_t i = TT_ENGINE_SPEED_SHIFT(2); i < TT_ENGINE_SPEED_FULL && sensor.front;
		     i += 2)
		{
